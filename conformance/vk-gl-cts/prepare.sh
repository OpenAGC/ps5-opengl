...
release=067e8832315e79817ede1c4863804e440f5d1c80 # opengl-cts-4.6.8.1

test -d "$cts/.git"
actual=$(git -C "$cts" rev-parse HEAD)
if [[ $actual != "$development" && $actual != "$release" ]]; then
    printf 'Unsupported VK-GL-CTS revision: %s\n' "$actual" >&2
    exit 2
fi
if [[ $actual == "$release" ]]; then
    # Keep release test bodies untouched; local test fixes are not approved waivers.
    git -C "$cts" diff HEAD --quiet -- \
        external/openglcts/modules/common/glcShaderNegativeTests.cpp || {
        printf 'Release CTS contains a modified negative-shader test.\n' >&2
        exit 3
    }
fi
...
for patch in "$overlay"/patches/*.patch; do
    if [[ $actual == "$release" && $(basename "$patch") == 0005-* ]]; then
        continue
    fi

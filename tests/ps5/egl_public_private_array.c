...
int main(void) {
    ...
    for (unsigned test=0;test<5;++test) {
        ...
        printf("[ps5-private-array] bytes=%u compile=%d scratch=%u code=%zu\n",
            words*4,rc,compiled.metadata.scratch_bytes_per_wave,compiled.machine_code_size);
        fflush(stdout);
        ...
        rc=ps5_agc_compute_execute(screen,&compiled,resources[0],resources+1,2,groups);
    ...
    for (unsigned i=0;i<3;++i) pipe_resource_reference(&resources[i],NULL);

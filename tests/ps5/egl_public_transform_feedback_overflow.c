...
#define CAPTURE_VERTICES 6
#define CAPTURE_BYTES (CAPTURE_VERTICES * 3 * sizeof(float))

_Static_assert(CAPTURE_OFFSET % sizeof(GLuint) == 0 &&
               CAPTURE_BYTES == 72,
               "transform-feedback range must be dword aligned and exact");
...
compile_shader(GLenum type, const char *source, GLuint *result)
{
   ...
   if (!ok) {
      ...
      printf("[ps5-egl-xfb-overflow] compile type=0x%x log=%.*s\n",
             type, length, log);
...
main(void)
{
   static const char *vertex_source =
      ...
      "out vec3 captured;\n"
      "void main() {\n"
      "  captured = vec3(a_position, float(gl_VertexID));\n"
   ...
   static const float vertices[18] = {
      ...
      -0.2f, -0.8f,  0.2f, -0.8f,  0.0f, -0.4f,
       0.4f, -0.8f,  0.8f, -0.8f,  0.6f, -0.4f,
   };
   static const float expected[CAPTURE_VERTICES * 3] = {
      -0.8f, -0.8f, 0.0f, -0.4f, -0.8f, 1.0f,
      -0.6f, -0.4f, 2.0f, -0.2f, -0.8f, 3.0f,
       0.2f, -0.8f, 4.0f,  0.0f, -0.4f, 5.0f,
   };
   ...
   if (mapped) {
      ...
      values_ok = !memcmp(mapped + CAPTURE_OFFSET, expected, CAPTURE_BYTES);
      suffix_ok = !memcmp(mapped + CAPTURE_OFFSET + CAPTURE_BYTES,
                          (const GLubyte *)initial + CAPTURE_OFFSET +
                             CAPTURE_BYTES,
                          OUTPUT_BYTES - CAPTURE_OFFSET - CAPTURE_BYTES);
      memcpy(first, mapped + CAPTURE_OFFSET, sizeof(first));
      memcpy(last, mapped + CAPTURE_OFFSET + CAPTURE_BYTES - sizeof(last),
             sizeof(last));
   }
...
cleanup:
   ...
   printf("[ps5-egl-xfb-overflow] result=%d\n", passed ? 0 : 1);
   return passed ? 0 : 1;
}

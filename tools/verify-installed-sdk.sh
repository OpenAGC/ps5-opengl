...
# Assemble and verify the relocatable PS5 OpenGL 4.6 consumer package.
...
prefix=$(realpath -m -- "${1:-$root/build/sdk/ps5-opengl-gl46}")

PS5_PAYLOAD_SDK="$sdk" \
    bash "$root/toolchain/install-ps5-opengl-gl46.sh" "$prefix"
...
printf 'installed-sdk: PASS prefix=%s consumer=core33-triangle commands=657\n' \
    "$prefix"
